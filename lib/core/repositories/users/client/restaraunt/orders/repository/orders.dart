...
class OrdersRepository implements AbstractOrdersRepository {
  ...
  Future<List<Order>> getOrdersList() async {
    ...
    // try {
    //   // ordersList = await _fetchOrdersListFromApi(); // Removed as /orders is not a server endpoint
    //   // final ordersMap = {for (var e in ordersList) e.id: e};
    //   // await ordersBox.putAll(ordersMap);
    // } catch (e, st) {
    //   GetIt.instance<Talker>().handle(e, st);
    //   // ordersList = ordersBox.values.toList(); // Already fetched
    // }
  ...
  void _connectToWebSocket() {
    ...
    if (token == null) {
      GetIt.I<Talker>().error('WebSocket: Access token is null, cannot connect.');
      _ordersStreamController.addError(Exception('Access token is null'));
      return;
    }
    final wsUrl = Uri.parse('$apiSiteUrl/orders/active/ws'.replaceFirst('http', 'ws'));
      ...
      (message) {
        try {
          ...
          if (decodedMessage is List) {
            updatedOrdersList = decodedMessage.map((item) => Order.fromJson(item as Map<String, dynamic>)).toList();
            ...
            // If it's a single new order, we'd ideally merge it with the existing list from ordersBox or last emitted list.
            ...
            var currentOrders = ordersBox.values.toList();
            final existingOrderIndex = currentOrders.indexWhere((o) => o.id == newOrder.id);
            ...
            GetIt.I<Talker>().warning('WebSocket: Unknown message format: $message');
            return;
          }
          updatedOrdersList.sort((a, b) => b.createdAt.compareTo(a.createdAt));
          ordersBox.putAll({for (var order in updatedOrdersList) order.id: order});
          _ordersStreamController.add(updatedOrdersList);
        } catch (e, st) {
          GetIt.I<Talker>().handle(e, st, 'Error processing WebSocket message in Repository');
          _ordersStreamController.addError(e);
        }
      },
      onError: (error, StackTrace stackTrace) {
        GetIt.I<Talker>().handle(error, stackTrace, 'WebSocket error in Repository');
        _ordersStreamController.addError(error);
        _channel = null; // Ensure channel is nullified before attempting to reconnect
        _channelSubscription = null;
        GetIt.I<Talker>().info('WebSocket error. Attempting to reconnect in 5 seconds...');
        Future.delayed(const Duration(seconds: 5), () {
          if (_ordersStreamController.hasListener) { // Only reconnect if there are still listeners
             _connectToWebSocket();
          }
        });
      },
      onDone: () {
        GetIt.I<Talker>().info('WebSocket connection closed in Repository. Attempting to reconnect in 5 seconds...');
        _channel = null; // Ensure channel is nullified before attempting to reconnect
        _channelSubscription = null;
        Future.delayed(const Duration(seconds: 5), () {
          if (_ordersStreamController.hasListener) { // Only reconnect if there are still listeners
  ...
  Future<List<Order>> _fetchOrdersListFromApi() async {
    try {
  ...
  // Ensure stream is closed when repository is disposed if GetIt supports disposal, 
  // or manage lifecycle from where OrdersRepository is created/disposed.
  // For now, manual call to closeOrdersStream is expected from BLoC's close method.
}